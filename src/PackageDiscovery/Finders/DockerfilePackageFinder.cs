...
namespace PackageDiscovery.Finders
{
    ...
    public sealed class DockerfilePackageFinder : IReferencedPackageFinder
    {
        public const string Moniker = "Docker";

        private static readonly Regex PodRegex = new Regex(
            @"\bFROM\s+(?<id>[^@\s]+)(@(?<version>[^\s]+))?"
        );

        public IReadOnlyCollection<Package> FindReferencedPackages(DirectoryInfo directory)
        {
            return directory
                .GetFiles("Dockerfile", SearchOption.AllDirectories)
